...
export default function JavariOSPage() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      const res = await fetch('https://javari-ai.vercel.app/api/autonomy/status', { cache: 'no-store' })
      if (!res.ok) return
  ...
  const send = useCallback(async (override?: string) => {
    ...
    fetch('https://javari-ai.vercel.app/api/javari/learning/update', {
      method: 'POST',
    ...
    try {
      if (mode === 'council') {
        ...
        const res  = await fetch('https://javari-ai.vercel.app/api/javari/team', {
        ...
        const res  = await fetch('https://javari-ai.vercel.app/api/javari/chat', {
  ...
  const runLoop = useCallback(async () => {
    ...
    setExecPulse(true)
    try {
      const res  = await fetch('https://javari-ai.vercel.app/api/autonomy/loop')
      const data = await res.json()
