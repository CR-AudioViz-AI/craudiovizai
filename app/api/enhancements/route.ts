// =============================================================================
// CR AUDIOVIZ AI - CENTRALIZED ENHANCEMENTS API
// =============================================================================
// Complete enhancement request management with Javari AI analysis
// All enhancement requests from the platform centralized here
...
interface EnhancementCreate {
  ...
  user_email?: string;
  user_name?: string;
  source_app?: string; // 'javari', 'tools', 'games', 'platform', 'admin', etc.
}
...
async function analyzeEnhancement(enhancement: any): Promise<AIAnalysis> {
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 2048,
      system: `You are Javari, the AI assistant for CR AudioViz AI platform. You specialize in analyzing enhancement requests and providing detailed implementation plans.

The CR AudioViz AI platform includes:
- **Javari AI**: Advanced AI chat assistant with autonomous capabilities
- **60+ Creative Tools**: Logo studio, ebook creator, music builder, PDF builder, etc.
- **1,200+ Games**: Browser-based games collection
- **Admin Dashboards**: Analytics, user management, billing, support
- **Credits System**: Stripe & PayPal integration for payments
- **Multi-tenant Architecture**: Supporting multiple apps under one platform

Tech Stack:
- Frontend: Next.js 14, React, TypeScript, Tailwind CSS
- Backend: Supabase (PostgreSQL), Vercel Edge Functions
- AI: Claude API, OpenAI API, various ML models
- Payments: Stripe, PayPal
- Infrastructure: Vercel, GitHub, multiple domains

Analyze enhancement requests thoroughly and provide actionable implementation plans.

Respond with JSON only in this exact format:
{
  "implementation_plan": "Detailed step-by-step plan with specific technical details and file changes",
  "estimated_effort": "X hours/days/weeks with breakdown by phase",
  "complexity": "trivial|simple|moderate|complex|very_complex",
  "potential_impacts": {
    "positive": ["List of benefits and improvements"],
    "negative": ["List of potential downsides or trade-offs"],
    "risks": ["List of risks with mitigation strategies"]
  },
  "dependencies": ["List of required dependencies, APIs, services, or prerequisites"],
  "recommendations": "Your professional recommendation including alternatives if applicable",
  "feasibility_score": 0-100,
  "priority_recommendation": "critical|high|medium|low with reasoning"
}`,
      messages: [{
        role: 'user',
        content: `Analyze this enhancement request for the CR AudioViz AI platform:

**Title:** ${enhancement.title}
**Category:** ${enhancement.category}
**Current Priority:** ${enhancement.priority}
**Source App:** ${enhancement.source_app || 'platform'}
...
Provide a comprehensive analysis with a detailed implementation plan that Roy (the founder/CEO) can review and approve.`
      }]
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    await supabase.from('enhancement_activity').insert({
      ...
      new_value: { status: 'submitted', source_app: body.source_app }
    });
    ...
    await supabase.from('enhancement_comments').insert({
      ...
      author_type: 'system',
      author_name: 'System',
      content: `💡 Enhancement request ${enhancement.request_number} submitted.\n📍 Source: ${body.source_app || 'platform'}\n\n🤖 Javari is analyzing your request and will provide a detailed implementation writeup shortly.`
    });
    
    // Start AI analysis (async - don't wait)
    const analysisPromise = (async () => {
      ...
      // Add analysis comment with full writeup
...
📧 *Admin will be notified for review.*`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: allEnhancements } = await supabase
      .from('enhancement_requests')
      .select('status, category, source_app, approval_status, upvotes');
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      await supabase.from('enhancement_comments').insert({
        enhancement_id: existing.id,
        author_type: 'admin',
        author_name: body.assigned_to || 'Admin',
        content: `🛠️ **Development Started**\n\nThis enhancement is now in active development.\n\n👤 Assigned to: ${body.assigned_to || 'Development Team'}\n📅 Target delivery: ${body.estimated_delivery || 'TBD'}`
      });
      ...
      await supabase.from('enhancement_comments').insert({
        enhancement_id: existing.id,
        author_type: 'admin',
        author_name: body.deployed_by || 'Admin',
        content: `🚀 **Enhancement Deployed!**\n\nThis enhancement has been successfully deployed to production.\n\n${body.implementation_notes || ''}\n\n*Thank you for your feedback!*`
      });
    }
...
export async function PUT(request: NextRequest) {
  try {
      ...
      if (existingVote) {
        if (existingVote.vote_type === vote_type) {
          ...
          if (vote_type === 'up') {
            await supabase
              .from('enhancement_requests')
              .update({ upvotes: Math.max(0, (enhancement?.upvotes || 0) - 1) })
              .eq('id', enhancementId);
          } else {
            await supabase
              .from('enhancement_requests')
              .update({ downvotes: Math.max(0, (enhancement?.downvotes || 0) - 1) })
              .eq('id', enhancementId);
          }
    ...
    if (action === 'comment') {
      ...
      await supabase
        .from('enhancement_requests')
        .update({ comment_count: (enhancement?.comment_count || 0) + 1 })
        .eq('id', enhancementId);
